...
def get_straight_line_angle(line: LineString, in_degree: bool = True) -> float:
    """
    计算边的角度, return angle in range [-90, 90]
    """
...
def get_angle_by_coords(coord1: Tuple[float, float], coord2: Tuple[float, float], in_degree: bool = True) -> float:
    if coord1 == coord2:
        raise ValueError("coord1 and coord2 should not be equal")
    line = LineString([coord1, coord2])
    return get_straight_line_angle(line, in_degree=in_degree)


def get_polygon_angle_by_bounding_box(polygon: Polygon, simplify_tolerance: float = 1e-6,
                                      in_degree: bool = True) -> float:
    """
    计算polygon的最小外接矩形的长边的角度, return angle in range [-90, 90]
    """
    rotated_box_coords = list(polygon.minimum_rotated_rectangle.simplify(simplify_tolerance).exterior.coords)
    ...
    return get_angle_by_coords(*longest_edge, in_degree=in_degree)
